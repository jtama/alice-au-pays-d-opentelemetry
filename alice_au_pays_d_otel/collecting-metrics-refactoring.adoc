...
[.notes]
--
La beauté de l'approche c'est que tout nouveau pod java déployé sera automatiquement pris en compte grace à l'ajout du sidecar.
...
[.notes]
--
C'est même pas écrit en petit avec une étoile, non, non c'est gros PLAF en plein milieux !
--
...
Jamais vous vous diriez je vais je me mettre à exploiter un cluster kube mais sans lire la doc... Donc je vais lire la doc.
...
Du coup j'ai aussi activé le processor k8sAttributeProcessor sur le collecteur maitre pour récupérer le reste des metadata et annotation déjà présent sur mes déploiements.
--
...
J'active la détection par variable d'environnement. Je fais tout le travail côté kube,  je ne vous le met pas dans les slides mais je vous donnerais le lien vers un dépôt qui contient tout ce qu'il faut.

Si vous faites du kube, regarder la downward API, c'est très pratique.
--

[.standard.columns.transparency]
=== !
...
[.standard.column.has-text-right]
--
Kubernetes
[horizontal.small]
...
[.notes]
--
Je ne montre pas tout, mais j'ai fait la même choses pour le reste de la stack, pas uniquement pour nos devs.
--
